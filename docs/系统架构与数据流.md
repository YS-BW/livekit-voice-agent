...
> 📌 文档目标：基于当前 `my-agent` 实际配置，整理端到端数据流、服务位置、关键时延路径，以及 `AgentSession(...)` 参数对应的优化点，便于联调与排障。
...
sequenceDiagram
    ...
    AG->>LK: 发布 AI 音轨（≈10-30ms）
    LK->>U: 下行 AI 语音播放

    opt 用户打断（Barge-in）
        U->>LK: 用户重新说话
        LK->>AG: 新语音 + 状态变化\n触发常见≈200ms（受 min_interruption_duration 影响）
        AG-->>AG: 中断当前 speech（取消该轮 LLM/TTS 未完成任务）
        AG->>STT: 进入下一轮识别
    end
```

> 💡 说明：打断不是只停 TTS，而是会中断当前整轮回复；该轮未完成的 LLM 与 TTS 任务都会被取消。  
> 📏 延迟口径：上图为工程经验区间，用于排障与容量预估；实际值请以 `log/*.jsonl` 指标为准。
